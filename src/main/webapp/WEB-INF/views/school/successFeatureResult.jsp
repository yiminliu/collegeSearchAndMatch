        ...
        <c:if test="${!empty schoolList}">
           <table class="datatable" style="font-size: 80%">
             <tr style="background-color: Ivory;">
                <th>School</th>
                <c:choose>
                   <c:when test="${not empty title && title eq '100 Colleges With Lowest TOEFL Score Requirement'}">
                      <th>TOEFL Score(Min/Avg)</th>
                   </c:when>
                   <c:when test="${not empty title && title eq '100 Colleges With Highest Acceptance Rate'}">
                      <th>Accept Rate</th>
                   </c:when>
